...
public class CsvDecoder {
    ...
    public boolean startNewLine() throws IOException {
        ...
        if (_skipEmptyLines && skippingBlankLinesReachedEOF()) {
            return false;
        }



        if (_allowComments&&_inputBuffer[_inputPtr] == '#' && skippingCommentsReachedEOF()) {
            return false;
        }
    ...
    /**
     * if we are on the start of a line comment, skip the rest of the line(s)
     *
     * @return true if we reached the end of the input
     * @throws IOException
     */
    private boolean skippingCommentsReachedEOF() throws IOException {
    ...
    public void skipLeadingEmptyLines() throws IOException {
        if (_skipEmptyLines) {
            skippingBlankLinesReachedEOF();
        }
    }

    /**
     * skip blank lines until we reach a non empty line or the end of input
     * if comments are ignored, they are also skipped here
     *
     * @return true if end of input was reached
     * @throws IOException
     */
    protected boolean skippingBlankLinesReachedEOF() throws IOException {
        int backupInputPtr = _inputPtr;
        while (hasMoreInput()) {
            ...
            if (ch == '\r' || ch == '\n') {
                _pendingLF = ch;
                _handleLF();
                // move the backup pointer to the start of the new line
                backupInputPtr = _inputPtr;
                continue;
            }

            if (_allowComments && ch == '#' ) {
                if (skippingCommentsReachedEOF()) {
                    return true; // end of input
                }
                continue;
            }

            // rollback to last non blank start line
            _inputPtr = backupInputPtr;
            return false; //
        }
        return true; // end of input
    }
